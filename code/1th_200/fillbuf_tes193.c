...
unsigned char myvivi_cur_bars[8][3];

/* Maximum number of bars are 10 - otherwise, the input print code
   should be modified */
static struct bar_std myvivi_bars[] = {
...
static void myvivi_precalculate_bars(int input)
{
	...
	for (k = 0; k < 8; k++) {
		r = myvivi_bars[input].bar[k][0];
		g = myvivi_bars[input].bar[k][1];
		b = myvivi_bars[input].bar[k][2];
		is_yuv = 0;
		...
		if (is_yuv) {
			myvivi_bars[k][0] = TO_Y(r, g, b);	/* Luma */
			myvivi_bars[k][1] = TO_U(r, g, b);	/* Cb */
			myvivi_bars[k][2] = TO_V(r, g, b);	/* Cr */
		} else {
			myvivi_bars[k][0] = r;
			myvivi_bars[k][1] = g;
			myvivi_bars[k][2] = b;
