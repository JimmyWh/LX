						...
						enum v4l2_field field)
{
	...
	vb->field  = myvivi_format.fmt.pix.field;
...
{
	return videobuf_streamoff(&myvivi_vb_vidqueue);
}
...
{
	return videobuf_mmap_mapper(&myvivi_vb_vidqueue, struct vm_area_struct * vma);
}
...
static void myvivi_timer_function(unsigned long data)
{
	...
	buf = list_entry(myvivi_vb_local_queue.next,
