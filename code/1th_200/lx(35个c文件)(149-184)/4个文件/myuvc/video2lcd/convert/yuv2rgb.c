...
int isSupportYuv2Rgb(int iPixelFormatIn,
				int iPixelFormatOut)
{
...
int Yuv2RgbConvert(PT_VideoBuf ptVideoBufIn,
	PT_VideoBuf ptVideoBufOut)
{
	...
	if (ptVideoBufOut->iPixelFormat == V4L2_PIX_FMT_RGB565)
	{
		...
		Pyuv422torgb565(ptPixelDatasOut->aucPixelDatas,
	...
	else if(ptVideoBufOut->iPixelFormat == V4L2_PIX_FMT_RGB32)
	{
		...
		Pyuv422torgb32(ptPixelDatasOut->aucPixelDatas,
			ptPixelDatasOut->aucPixelDatas, 
			ptPixelDatasOut->iWidth,ptPixelDatasOut->iHeight);
	}
	return 0;
...
{
	initLut();
	return RegisterVideoConver(&g_tYuv2RgbConvert);
}
